...
3. **백엔드 로직2 (검증/평가)**: 문단별로 링크 최대 5개까지 ScrapingBee 병렬 크롤링 → 문단+원문링크 텍스트 → GPT-5 호출로 신뢰도 산정/근거 요약 → 문단ID별로 부분 푸시 → UI가 실시간 반영.
...
- 문단 하나를 기준으로
    ...
    2. 각 링크 본문 요약
    3. "문단 내용"과 "링크 요약들"을 GPT-5한테 던져서
        - 일치도 / 누락 / 과장 / 반전 여부 체크
        - 최종 신뢰도(상/중/하) 매기기
    4. 그 결과를 UI로 스트리밍/푸시
...
3. **프롬프트 구성 (GPT-5)**
    ...
    - 유저:
        ...
        - 지시: "문단이 출처에 근거하고 있으면 high, 일부만이면 medium, 대부분 없으면 low"
    - 모델 응답 포맷을 **JSON 강제**
        ...
        ```json
        {
          ...
          "rationale": "...",
          "per_source": [
            { "url": "...", "match": "full|partial|none", "notes": "..." }
          ]
        }
        ```
...
5. **상태 저장**
    - DB에 `doc_id + paragraph_id + confidence + reasoning + created_at`
